...
var FormBuilder = function FormBuilder(opts, element) {
  ...
  var prepFieldVars = function prepFieldVars($field) {
    ...
    if (isNew && _utils2.default.inArray(field.type, ['text', 'number', 'file', 'date', 'select', 'textarea', 'autocomplete', 'remotecomplete'])) {
  ...
  var fieldOptions = function fieldOptions(fieldData) {
    ...
    if (!values || !values.length) {
      ...
      if (_utils2.default.inArray(type, ['checkbox-group', 'checkbox'])) {
        defaultOptCount = [1];
      } else if (_utils2.default.inArray(type, ['flipswitch'])) {
        defaultOptCount = [1, 2];
      }
    ...
    var allOptions = type == 'flipswitch' ? [options] : [options, optionActionsWrap];
    var optionsWrap = m('div', allOptions, { className: 'sortable-options-wrap' });
  ...
  var defaultFieldAttrs = function defaultFieldAttrs(type) {
    ...
    var noValFields = ['header', 'paragraph', 'file', 'autocomplete', 'remotecomplete', 'split'].concat(d.optionFields);
    ...
    var typeAttrsMap = {
      ...
      checkbox: ['required', 'label', 'description', 'toggle', 'inline', 'className', 'name', 'access', 'other', 'options', 'dataset', 'listen', 'source', 'target'],
      ...
      flipswitch: ['required', 'label', 'description', 'className', 'name', 'other', 'options', 'dataset', 'listen', 'source', 'target']
    };
    typeAttrsMap['checkbox-group'] = ['required', 'label', 'description', 'toggle', 'inline', 'className', 'name', 'access', 'other', 'subtype', 'options', 'dataset', 'listen', 'source', 'target'];
    typeAttrsMap['radio-group'] = ['required', 'label', 'description', 'toggle', 'inline', 'className', 'name', 'access', 'other', 'subtype', 'options', 'dataset', 'listen', 'source', 'target'];
    ...
    if (_utils2.default.inArray(type, ['header', 'paragraph', 'button', 'split'])) {
  ...
  var advFields = function advFields(values) {
    ...
    var advFieldMap = {
      ...
      dataset: function dataset() {
        return datasetField(values);
      },
      listen: function listen() {
        return listenField(values);
      },
      source: function source() {
        return sourceField(values);
      },
      target: function target() {
        return targetField(values);
      },
  ...
  function inputUserAttrs(name, attrs) {
    ...
    var optionInputs = ['checkbox', 'checkbox-group', 'radio-group', 'flipswitch'];
  ...
  var selectAttribute = function selectAttribute(attribute, values, optionData) {
    ...
    var visibility = attribute === 'subtype' && values.type === 'header' ? 'none' : 'block';
  ...
  var requiredField = function requiredField(fieldData) {
    ...
    var noRequire = ['header', 'paragraph', 'button', 'split'];
    var noMake = [];
    var requireField = '';
  ...
  var datasetField = function datasetField(fieldData) {
    var type = fieldData.type;

    var noRequire = ['header', 'paragraph', 'button', 'split'];
    var noMake = [];
    var field = '';
    var options = opts.dataset.map(function (data) {
  ...
  var listenField = function listenField(fieldData) {
    var type = fieldData.type;
  ...
  var sourceField = function sourceField(fieldData) {
    var type = fieldData.type;
  ...
  var targetField = function targetField(fieldData) {
    var type = fieldData.type;
  ...
  $stage.on('change', '[name="dataset"]', function (e) {
    var $holder = $(e.target).closest('div.frm-holder');
    var val = $(e.target).val();
    $holder.find('[name="listen"]').val('').empty();
    $holder.find('[name="source"]').val('').empty();
    $holder.find('[name="target"]').val('').empty();

    var buildOptions = function buildOptions(e, o) {
    ...
    if (val) {
      ...
      var sourceList = opts.dataset.filter(function (item) {
      ...
      var options = $(e.target).closest('div.build-wrap').data('formBuilder').actions.getData().map(function (data) {
...
var optionFields = exports.optionFields = ['select', 'checkbox-group', 'checkbox', 'radio-group', 'autocomplete', 'flipswitch'];
...
var defaultOptions = exports.defaultOptions = {
  ...
  append: false,
  actionButtons: [],
  controlOrder: ['autocomplete', 'button', 'checkbox', 'checkbox-group', 'date', 'file', 'header', 'hidden', 'paragraph', 'number', 'radio-group', 'select', 'flipswitch', 'text', 'textarea', 'split'],
  locale: 'zh-CN',
  dataType: 'json',
...
_control3.default.register(['date', 'time', 'datetime-local'], controlText, 'date');
_control3.default.register(['number', 'money'], controlText, 'number');
_control3.default.register(['text', 'password', 'email', 'tel'], controlText, 'text');
...
_text2.default.register('file-in-tab', _text2.default, 'file');
...
var controlParagraph = function (_control) {
  ...
  controlParagraph.prototype.configure = function configure() {
...
_control3.default.register(['paragraph', 'header', 'split'], controlParagraph);
...
_control3.default.register(['h1', 'h2', 'h3'], controlParagraph, 'split');
...
var controlSelect = function (_control) {
  ...
  controlSelect.prototype.configure = function configure() {
  ...
  controlSelect.prototype.build = function build() {
    ...
    var isFlipswitch = type == 'flipswitch';
...
_control3.default.register(['vertical', 'horizontal'], controlSelect, 'checkbox-group');
_control3.default.register(['vertical', 'horizontal'], controlSelect, 'radio-group');
_control3.default.register(['single', 'multiple', 'filterable'], controlSelect, 'select');
